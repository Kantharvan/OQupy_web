...
function localWallClockToISO(day: Date, time: string): string {
  // time is "HH:mm" — treat as LOCAL time on the picked day, convert to UTC ISO
            ...
            <div className={`${t.cardBox} overflow-hidden`}>
              ...
              <div className="p-6">
                ...
                <p className={`font-bold text-lg ${t.brandText} mt-3`}>
                  ₹{studio.price}
                  <span
                    className={`text-sm font-normal ${t.textMuted}`}
                  >
                    /hr
                  </span>
                </p>
                {studio.description && (
                  <p className={`text-sm ${t.textSecondary} mt-4`}>
                    {studio.description}
                  </p>
                )}
                {studio.amenities.length > 0 && (
                  <div className="flex gap-2 flex-wrap mt-4">
                    {studio.amenities.map((a) => (
                      <span
                        key={a}
                        className={`text-xs px-2.5 py-1 rounded-full bg-bg-input ${t.textSecondary}`}
                      >
                        {a}
                      </span>
                    ))}
              ...
              {isAuthLoading ? null : !user ? (
                <div className="text-center py-4">
                  <p className={`text-sm ${t.textMuted} mb-3`}>
                    Sign in to book this studio.
                  </p>
                  <Link
                    href="/login"
                    className={`h-10 px-5 ${t.btnPrimary} text-sm inline-flex items-center`}
                  >
                    Sign in
                  </Link>
                </div>
...
function BookingFlow({
  ...
  const bookingType: BookingType =
    userRole === "student" ? "student_practice" : "instructor_event";
  ...
  const disabledDayMatcher = useCallback((day: Date) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (
        ...
        setFormError(
          "This slot was just taken by someone else. Pick another time.",
        );
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      {/* Section 1 — Date */}
      <section>
        <h3 className={`text-xs uppercase ${t.textMuted} mb-2`}>
          1. Pick a date
        </h3>
        ...
        <section>
          ...
          {isFetchingAvailability ? (
            ...
            <p className={`text-sm ${t.textMuted}`}>
              Studio is closed on {format(selectedDay, "EEEE")}. Pick a
              different day.
            </p>
          ) : slots.length === 0 ? (
          ...
          {selectedTime && (
            <div className="mt-4">
              <label
                className={`text-xs uppercase ${t.textMuted} mb-2 block`}
              >
                Duration
              </label>
              <div className="flex gap-2 flex-wrap">
                {DURATION_OPTIONS.map((d) => (
                  <button
                    ...
                    className={
                      durationHours === d ? t.timeSlotSelected : t.timeSlot
                    }
                  >
                    {d}h
                  </button>
              ...
              {wouldConflict && (
                <p className="text-red-400 text-xs mt-2">
                  This duration overlaps a busy window. Pick a shorter slot or
                  a different time.
                </p>
              )}
            </div>
          ...
          <div className={`${t.cardBox} p-4 flex flex-col gap-3`}>
            <div className="flex items-center justify-between">
              <span className={`text-sm ${t.textSecondary}`}>
                {durationHours}h × ₹{studio.price}/hr
              </span>
              <span className={`font-bold text-lg ${t.brandText}`}>
            ...
            <div className="flex gap-2 flex-wrap">
              {PAYMENT_METHODS.map((m) => (
                <button
                  ...
                  className={
                    paymentMethod === m.value ? t.timeSlotSelected : t.timeSlot
                  }
                >
                  {m.label}
                </button>
            ...
            <p className={`text-xs ${t.textMuted}`}>
              Payment via Razorpay is collected after the studio owner approves
              your booking. No charge now.
            </p>
          </div>
        </section>
      )}
      ...
      <button
        type="submit"
        disabled={!canSubmit}
        className={`h-11 ${t.btnPrimary} text-sm font-semibold`}
      >
